...
import {getBrokerDetails, getAgentSearches} from '../../api/userAdmin.api';
...
export const BrokerDetailsScreen = ({
  navigation,
  route,
}: AppScreenProps<'BrokerDetails'>) => {
  ...
  // The web's broker-property-search page lists broker searches via
  // /get-agent-searches (it accepts any user id), so we match that here.
  const searchesFetcher = useCallback(
    () =>
      brokerId
        ? getAgentSearches(brokerId, range.fromDatetime, range.toDatetime)
  ...
  const broker = useMemo(() => {
    ...
    if (__DEV__ && rawDetails) {
      console.log(
        '[BrokerDetails] getBrokerDetails:',
        JSON.stringify(rawDetails)?.slice(0, 600),
      );
    }
    const businessRaw =
      d.revenue ??
            ...
            <>
              ...
              <View style={styles.metaGrid}>
                ...
                <View style={styles.metaCell}>
                  ...
                  <Text style={styles.metaValue}>${broker.business}</Text>
                </View>
              </View>
            </>
          )}
